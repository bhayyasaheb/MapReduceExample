...
public class Top5LossProductAgeWise {
	...
	/*
	 * Mapper Mapper runs on input retail data like D*
	 */
	public static class Top5ViableMapper extends Mapper<LongWritable, Text, Text, Text>
	{
		...
		protected void map(LongWritable key, Text value, Context context)throws IOException, InterruptedException 
		{
			...
			String myValue = product+","+age+","+loss;
			
			if(loss > 0)
			{
				// putting product as key and product,age,loss as value in mapper output
	...
	public static class Top5ViableReducer extends Reducer<Text, Text, NullWritable, Text>
	{
		...
		protected void reduce(Text key, Iterable<Text> values,Context context)throws IOException, InterruptedException 
		{
			...
			// in TreeMap key = loss and value = product,age,totalLoss
			 treeMap.put(new Long(totalLoss), new Text(myValue));
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "Using Tree Map Top 5 Loss Making Product Age Wise Partitioner");
		
		job.setJarByClass(Top5LossProductAgeWise.class);
