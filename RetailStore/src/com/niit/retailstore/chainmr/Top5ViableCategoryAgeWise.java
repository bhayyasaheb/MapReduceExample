...
public class Top5ViableCategoryAgeWise {
	...
	public static class FirstMapper extends Mapper<LongWritable, Text, Text, Text>
	{
		...
		protected void map(LongWritable key, Text value, Context context)throws IOException, InterruptedException 
		{
			...
			// passing category as key and value as value as Mapper output
	...
	public static class FirstReducer extends Reducer<Text, Text, LongWritable, Text>
	{
		...
		protected void reduce(Text key, Iterable<Text> values,Context context)throws IOException, InterruptedException 
		{
			...
			// saving category and age to myKey here key is category with \t as delimiter
			 String myKey = key +"\t"+age;
			 
			 if(viable > 0)
			 {
				 // writing reducer output when viable is +ve  
				 // key is difference between sales - cost and value is category and age as Reducer output
				 context.write(new LongWritable(viable), new Text(myKey));
			 }
	...
	public static class SortMapper extends Mapper<LongWritable, Text, DoubleWritable, Text>
	{
		...
		protected void map(LongWritable key, Text value,Context context)throws IOException, InterruptedException 
		{
			...
			// viable is token[0] as sortKey
			double sortKey =  Double.parseDouble(token[0]);
			
			// here token[1] is category & token[2] is age
			String sortValue = token[1]+"\t"+token[2];
			
			// Mapper output key is viable and output value is category and age
	...
	/*
	 * Partitioner 2 on SortMapper output
	 */
	...
	public static class SortReducer extends Reducer<DoubleWritable, Text, Text, DoubleWritable>
	{
		...
		protected void reduce(DoubleWritable key, Iterable<Text> values,Context context)throws IOException, InterruptedException
		{
			if(count < 5)
			{
				for(Text val :values)
				{
					// Reducer output key is category & age and output value is viable
					// writing for only top5 viable
	...
	public static void main(String[] args) throws Exception {
		...
		Job job1 = Job.getInstance(conf,"Using Chain MR Top 5 Viable Category Age wise");
		
		job1.setJarByClass(Top5ViableCategoryAgeWise.class);
		...
		Path outputPath1 = new Path("FirstMapper2"); // This will be stored in hdfs like /user/hduser/FirstMapper2
		...
		Job job2 = Job.getInstance(conf,"Using Chain MR Top 5 Viable Category Age wise");
		
		job2.setJarByClass(Top5ViableCategoryAgeWise.class);
